    ...
    if (isset($_POST['create_tables'])) {
        ...
        try {
            ...
            foreach ($statements as $statement) {
                ...
                // Remove comentários de linha do statement
                $lines = explode("\n", $statement);
                $cleanLines = [];
                foreach ($lines as $line) {
                    $line = trim($line);
                ...
                try {
                    ...
                    if (!$shouldIgnore) {
                        ...
                        $_SESSION['statement_debug'][] = "ERRO: " . substr($errorMsg, 0, 100);
            ...
            if ($stmt->rowCount() === 0) {
                ...
                $error .= "<strong>Statement Debug:</strong><br>";
                if (isset($_SESSION['statement_debug'])) {
                    foreach (array_slice($_SESSION['statement_debug'], 0, 10) as $debug) {
                        $error .= htmlspecialchars($debug) . "<br>";
                    }
                }
                $error .= "<br><strong>Tabelas existentes no banco:</strong><br>" . (empty($allTables) ? "Nenhuma tabela encontrada" : implode(', ', $allTables));
                $error .= "<br><br><strong>Erros encontrados:</strong><br>" . (empty($errors) ? "Nenhum erro" : implode('<br>', $errors));
            } else {
                $step = 3;
            }
        } catch (PDOException $e) {
                ...
                <div class="card-body">
                    ...
                    <?php if ($showPasswordScreen): ?>
                        ...
                        <div class="text-center mt-3">
                            <a href="<?= $envConfig['APP_URL'] ?? '/mvc08' ?>" class="text-decoration-none">
                    ...
                    <?php elseif (!$needsInstallation): ?>
                        ...
                        <div class="text-center">
                            ...
                            <a href="<?= $_SESSION['db_config']['app_url'] ?? '/mvc08' ?>" class="btn btn-primary btn-lg">
